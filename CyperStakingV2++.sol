 ...
 */
contract CyberStakingRewards is RewardsDistributionRecipient, ReentrancyGuard{
    using SafeMath for uint256;
    CyberArms public CyberArmsRewards;
    ERC20 public MiddleGoldCoin;
    ERC20 public GoldCoin;
    ...
    mapping(uint256 => uint256) private _getPrice;
    ...
    constructor (
        ...
        address _MiddleGoldCoin,
        address _GoldCoin,
        address _CyberArmsRewards
    ) {
        MiddleGoldCoin = ERC20(_MiddleGoldCoin);
        GoldCoin = ERC20(_GoldCoin);
        CyberArmsRewards = CyberArms(_CyberArmsRewards);
    ...
    /**
     * @dev See {IStakingRewards-getPrice}.
     */
        ...
        returns (uint256)
    {
        ...
        if ( tokenId <= 6 && 4 <= tokenId) {
            price = tokenId*rewardRate*1000;
            return price;
        } else if (tokenId <= 3) {
            price = tokenId*rewardRate;
            return price;
        } else {
            price = tokenId.mul(1000).mul(rewardRate).add(
                    reward - tokenId
                    );
    ...
    /**
     * @dev See {IStakingRewards-choose}.
     */
    function choose (uint256 k ,uint256 tokenId) internal returns(uint256){
    ...
    function getNFT (uint256 k, uint256 tokenId) public nonReentrant updateReward(msg.sender){
        ...
        MiddleGoldCoin.transferFrom(msg.sender, address(this), reward);
        CyberArmsRewards.mint( msg.sender, tokenId, k);
    ...
    function getReward() public  nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            MiddleGoldCoin.approve(address(this), reward);
            MiddleGoldCoin.transferFrom(address(this), msg.sender,reward);
    ...
    function notifyRewardAmount(uint256 reward) external  onlyRewardsDistribution updateReward(address(0)) override{
        ...
        uint balance = MiddleGoldCoin.balanceOf(address(this));
