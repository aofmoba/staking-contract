...
contract CyberStakingRewards is RewardsDistributionRecipient, ReentrancyGuard, ERC1155Holder{
    ...
    function getNFT (uint256 k, uint256 tokenId) public nonReentrant updateReward(msg.sender){
        ...
        if(_balances[msg.sender].mul(100).div(_totalSupply) >= 50 )
        {
            require(rewardsDuration <=  block.timestamp.div(lastTimeRewardApplicable()),"Can not get NFT");
        }
    ...
    function setNFT(uint256 n, uint256 price) public{
        _price[n] = price;
    }
